...
public class BookService {
    ...
    public List<Book> getFreeBooks() throws IOException {
        System.out.println("🌐 CLIENT: Richiesta libri gratuiti al server");
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.isSuccessful() && response.body() != null) {
                ...
                System.out.println("✅ CLIENT: Ricevuti " + books.size() + " libri gratuiti dal server");
                if (!books.isEmpty()) {
    ...
    /**
     * Recupera libri più recensiti dal server
     */
    public CompletableFuture<List<Book>> getMostReviewedBooksAsync() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                return getMostReviewedBooks();
            } catch (Exception e) {
                System.err.println("❌ Errore durante il recupero dei libri più recensiti: " + e.getMessage());
                // Fallback: usa getFallbackBooks invece di getFeaturedBooks
                List<Book> fallbackBooks = getFallbackBooks();

                // Simula dati di recensioni per il fallback
                for (int i = 0; i < fallbackBooks.size(); i++) {
                    Book book = fallbackBooks.get(i);
                ...
                // Restituisci solo i primi 3 per coerenza
                return fallbackBooks.subList(0, Math.min(3, fallbackBooks.size()));
            }
        });
    }

    public List<Book> getMostReviewedBooks() throws IOException {
        System.out.println("🏆 CLIENT: Richiesta libri più recensiti al server");

        Request request = new Request.Builder()
                .url(SERVER_BASE_URL + "/books/most-reviewed")
                .get()
                .build();
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.isSuccessful() && response.body() != null) {
                ...
                System.out.println("✅ CLIENT: Ricevuti " + books.size() + " libri più recensiti dal server");
                for (Book book : books) {
                    System.out.println("   📊 " + book.getTitle() + " - " +
                            book.getReviewCount() + " recensioni, media: " + book.getAverageRating());
                }
    ...
    /**
     * Recupera libri meglio valutati dal server
     */
    public CompletableFuture<List<Book>> getTopRatedBooksAsync() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                System.err.println("❌ Errore durante il recupero dei libri meglio valutati: " + e.getMessage());
                // Fallback
                List<Book> fallbackBooks = getFallbackBooks();

                // Simula dati di valutazioni alte per il fallback
                for (int i = 0; i < fallbackBooks.size(); i++) {
                    Book book = fallbackBooks.get(i);
                ...
                // Restituisci 6 libri
                return fallbackBooks.subList(0, Math.min(6, fallbackBooks.size()));
            }
        });
    }

    public List<Book> getTopRatedBooks() throws IOException {
        System.out.println("⭐ CLIENT: Richiesta libri meglio valutati al server");

        Request request = new Request.Builder()
                .url(SERVER_BASE_URL + "/books/top-rated")
                .get()
                .build();
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.isSuccessful() && response.body() != null) {
                ...
                System.out.println("✅ CLIENT: Ricevuti " + books.size() + " libri meglio valutati dal server");
                for (Book book : books) {
                    System.out.println("   ⭐ " + book.getTitle() + " - media: " + book.getAverageRating() +
                            " (" + book.getReviewCount() + " recensioni)");
                }
