...
Per prima cosa, dobbiamo conoscere il posizionamento del file che ci interessa, chiamato **`Completo.sql`**, che si trova all'interno della cartella **`Database`**.  

Assicurati di avere a portata di mano il **percorso completo** di questo file (ad esempio: `/Users/TuoNome/Progetto_BABO/Database/Completo.sql`), poiché ti servirà nel passaggio 3.3. 
...
1. Apri il terminale (su Mac/Linux) o il **Prompt dei comandi** (su Windows) e inserisci il seguente comando per creare il database.
    Sostituisci `nome_database` con il nome che preferisci (es. `prova`). 

	```bash 
	      createdb -U postgres nome_database 
	```
...
3. Qualora non riuscissi con la riga di comando, puoi sempre usufruire dell'interfaccia grafica offerta da **pgAdmin**. 
  ...
  * Inserisci il nome del database (es. `prova`) nel campo "Database" e salva.
...
2.  Esegui il seguente comando, sostituendo il segnaposto con il nome che hai assegnato al database:

    `pg_restore -U postgres -d nome_database Completo.sql’
...
4. Si aprirà una finestra: nel campo "Filename", clicca sull'icona della cartella per navigare fino al file Completo.sql contenuto nella cartella Database.
